...
int n,m;
int free_frames_cnt,page_faults,page_accesses,swaps,min_active_processes;
Process* processes;
...
void swap_out_process(int process_number){
  ...
  if(p->searches_done<m)queue_enqueue(&swapped_out_queue,process_number);
...
void swap_in_process(){
  if(free_frames_cnt<ESSENTIAL_PAGES){
    return;
  }
  ...
  if(binary_search(process_number)){
    if(p->searches_done<m)queue_enqueue(&ready_queue,process_number);
    else{
      swap_out_process(process_number);
      swap_in_process();
    }
  ...
  while(1){
    ...
    if(p->is_active && p->searches_done<m){
      ...
      if(binary_search(curr_process)){
        if(p->searches_done<m)queue_enqueue(&ready_queue,curr_process);
        else{
...
void read_input(){
  ...
  for(int i=0;i<n;i++){
    ...
    fscanf(fp,"%d",&p->size);
    p->search_indices=(int*)malloc(p->size*sizeof(int));
    for(int j=0;j<m;j++){
